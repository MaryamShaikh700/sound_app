...
class _MyAppState extends State<MyApp> {
  ...
  void printRecordedAudioPath() {
    if (_path[_codec.index] != null) {
      print('Recorded audio path: ${_path[_codec.index]}');
    } else {
  ...
  Future<void> startPlayer(String audioFilePath) async {
    try {
      var codec = _codec;

      // Check the media type to determine how to start the player
      if (_media == Media.stream) {
      ...
      playerModule.logger.e('Error starting player: $err');
    }
  }
  ...
  void Function()? onStartPlayerPressed() {
    ...
    return (playerModule.isStopped) ? () => startPlayer(_path[_codec.index]!) : null;
  }
          ...
          body: Center(
            child: Column(
              children: [
                SizedBox(height: 50,),
                recorderSection,
                // ElevatedButton(onPressed: printRecordedAudioPath, child: Text('print')),
                playerSection
              ],
            )
          ),
